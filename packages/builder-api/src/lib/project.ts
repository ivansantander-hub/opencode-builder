...
import type { D1Adapter } from "./storage.js"

export class ProjectManager {
  constructor(private storage: D1Adapter) {}
  ...
  async get(id: string): Promise<Project | null> {
    const result = await this.storage.query<Record<string, unknown>>("SELECT * FROM project WHERE id = ?", [id])
  ...
  private parseRow(row: Record<string, unknown>): Project {
    return {
      ...
      status: String(row.status) as "active" | "archived" | "deleted",
